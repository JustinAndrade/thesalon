...
const SliderMenu = (props) => {
  ...
  return (
    <div
      className={
        "flex flex-column justify-center items-center bg-gold fixed top z-max w-100 ease" +
    ...
    >
      <Link
        to="/"
        className={
          "display ttu tracked white f3 no-underline menu__item pv5 hover-gray" +
          extraClasses
        }
      >
        {props.siteTitle}
      ...
      {props.extraLinks.map((navLink) => (
        <MultiLink
          to={navLink.to}
          className={
            "sans-serif ttu mid-gray f5 no-underline menu__item pv3 hover-white" +
            extraClasses
          }
        >
          {navLink.name}
